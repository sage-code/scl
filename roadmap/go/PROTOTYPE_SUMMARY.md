...
You requested an improved architecture that transforms the Go programming lab from a simple card-based hub into a structured learning platform with **progress tracking**. This has been successful created as a **prototype** in `/roadmap/go/`.

### New Architecture Pattern

```
Layer 1: Hub (Progress Table)
/roadmap/go/index.html
...
/roadmap/go/topic.html?topic=syntax
...
/roadmap/go/{topic_name}.json    [NEW - Per-topic sidebars]
/roadmap/go/{topic_name}.html    [EXISTING - Topic content]
/roadmap/go/topics.json          [NEW - Master topics list]
```
...
```
1. Visit /roadmap/go/index.html
   → Loads topics.json (12 topics)
   → Displays table with 12 rows (0% complete initially)
   
2. Click "Arrays" row
   → Navigate to /roadmap/go/topic.html?topic=arrays
...
4. Click "Next: Maps" in sidebar
   → Navigate to /roadmap/go/topic.html?topic=maps
...
6. Return to hub (/roadmap/go/index.html)
...
✓ Test 1: Hub Loads Correctly
  1. Open http://localhost:5500/roadmap/go/index.html
...
✓ Test 2: Topic Page Loads
  ...
  2. Navigate to http://localhost:5500/roadmap/go/topic.html?topic=syntax
...
✓ Test 6: Sidebar Sections
  1. Navigate to /roadmap/go/topic.html?topic=functions
...
```bash
/roadmap/go/ → template
/roadmap/julia/ → create (copy go structure)
/roadmap/dart/ → create (copy go structure)
