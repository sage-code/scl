...
```
Entry Point:
  http://localhost:5500/roadmap/go/index.html
...
- [ ] **Test 1.1**: Open `/roadmap/go/index.html`
...
- [ ] **Test 2.1**: Click "Syntax" topic in table
  - Expected: Navigate to `/roadmap/go/topic.html?topic=syntax`
...
- [ ] **Test 3.1**: Checkbox in hub
  - Return to `/roadmap/go/index.html`
...
- [ ] **Test 5.1**: Invalid topic URL
  - Manual type: `/roadmap/go/topic.html?topic=invalid_topic`
...
- [ ] **Test 5.3**: Deep linking
  - Copy URL: `http://localhost:5500/roadmap/go/topic.html?topic=functions`
...
2. **Next Phase**: Replicate to other programming languages
   ```
   /roadmap/julia/   (follow same pattern)
   /roadmap/dart/
   /roadmap/python/
   /roadmap/javascript/
   ... (15 languages total)
...
|-------|---------|----------|
| **404 Not Found** | "Failed to load topics.json" in console | Verify file exists: `/roadmap/go/topics.json` |
...
- Create `/roadmap/julia/` with same structure
- Create `/roadmap/dart/` with same structure
...
**Hub URL**: http://localhost:5500/roadmap/go/index.html
**Topic URL Example**: http://localhost:5500/roadmap/go/topic.html?topic=syntax
**Files to Check if Errors**:
- `/roadmap/go/topics.json` - Master list
- `/roadmap/go/{topic_name}.json` - Each navigation file
- `/roadmap/go/{topic_name}.html` - Each content file
- `/roadmap/go/topic.html` - Viewer template
