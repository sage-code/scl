...
- **Visual mode (SVG diagrams):** when a concept is spatial or relational — control flow, memory layout, type/class hierarchy, async timelines, build pipelines — include a GENERATED DIAGRAM. Convention:
  - Author a self-contained SVG: declare `viewBox`, responsive `width:100%`, minimal and technical (never decorative). Follow the dark-theme style spec in `manual/ARCHITECTURE.md` §Diagrams — non-negotiable: solid non-transparent canvas and boxes (`#0f172a` background / `#1e293b` fill), light high-contrast writing (`#e2e8f0` / `#94a3b8`) that is never dark on dark boxes, dark writing allowed ONLY inside a light highlight box (`#f59e0b` / `#3b82f6` / `#ef4444` / `#34d399`), UML/logic shapes only (rounded rect, rect, diamond, labeled arrow — never floating unboxed nodes), shapes aligned on one axis with >=12px gaps, never touching.
