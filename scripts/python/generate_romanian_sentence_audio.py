#!/usr/bin/env python3
"""Generate Romanian sentence audio files and attach reusable table controls.

This script scans roadmap/romanian/sentences.html, extracts the first column text
from each sentence table row, generates MP3 files with Edge TTS, and rewrites rows
...
def extract_sentences(soup: BeautifulSoup) -> list[str]:
    ...
    for row in soup.select("main table tbody tr"):
        ...
        if first_col is None:
            continue
        sentence = first_col.get_text(" ", strip=True)
        if sentence:
...
def rel_audio_href(file_name: str) -> str:
    return f"/roadmap/romanian/audio/sentences/{file_name}"
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Romanian sentence audio files")
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    html_file = repo_root / "roadmap" / "romanian" / "sentences.html"
    audio_dir = repo_root / "roadmap" / "romanian" / "audio" / "sentences"
